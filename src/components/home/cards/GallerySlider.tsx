...
export default function GallerySlider({
  ...
  function changePhotoId(newVal: number) {
    if (newVal >= 0 && newVal < images.length) {
      ...
      setFallbackIndex(0); // Reset fallback index
    }
  }
  ...
  const handleImageError = () => {
    ...
    if (fallbackIndex < FALLBACK_IMAGES.length - 1) {
      ...
      setImageError(false); // Reset error to try next fallback
    } else {
      setImageError(true);
    }
  };
        ...
        <div className={`w-full overflow-hidden ${galleryClass}`}>
          {onImageClick ? (
            <div
              ...
              className={`relative flex items-center justify-center ${ratioClass} cursor-pointer`}
        ...
        <>
          ...
          {images.length > 1 && (
            <>
              ...
              <div
                className={`flex items-center justify-center absolute  left-1/2 transform -translate-x-1/2 space-x-1.5  ${type === "card1" ? "bottom-2" : "bottom-16"} z-10`}
              >
                {images.map((_, i) => (
                  <button
