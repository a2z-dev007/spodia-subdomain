...
const BottomTab = () => {
  ...
  return (
    <>
      {/* Bottom Navigation */}
      <nav className={`fixed bottom-0 left-0 right-0 w-full max-w-full overflow-hidden z-[45] bg-white border-t border-gray-100 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] lg:hidden safe-area-bottom transition-transform duration-300 ${
        isVisible ? 'translate-y-0' : 'translate-y-full'
      }`}>
        <div className="flex items-center h-16 px-2 overflow-x-auto scrollbar-hide scroll-smooth justify-around w-full max-w-full">
          {tabs.map((tab) => {
