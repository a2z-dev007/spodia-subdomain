...
import { FooterShimmer } from "@/components/Footer/FooterShimmer";

const Footer = () => {
  const [activeIndex, setActiveIndex] = useState<number | null>(null);
  const [footerData, setFooterData] = useState<FooterApiResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const accordion = true; // toggle this to false for independent mode

  // Single API call for all footer data
  useEffect(() => {
          ...
          <div className="max-w-7xl mx-auto space-y-2 sm:space-y-3">
            {expandableSections.map((section, idx) => (
              <FooterExpandable
                ...
                defaultVisible={10}
                expanded={accordion ? activeIndex === idx : undefined}
                onToggle={
                  accordion
