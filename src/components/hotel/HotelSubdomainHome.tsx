...
export default async function HotelSubdomainHome({
  entityKey,
  initialHotelData,
}: Props) {
  ...
  const hotelData: ListingDetail | null =
    initialHotelData !== undefined
      ? initialHotelData
  ...
  const rawCover =
    hotelData?.images?.find((i) => i.cover_photo)?.file ||
    hotelData?.images?.[0]?.file;
  const heroImage = rawCover
    ? rawCover.startsWith("http")
      ? rawCover
      : `${IMAGE_BASE_URL}${rawCover.startsWith("/") ? "" : "/"}${rawCover}`
  ...
  const jsonLd = hotelJsonLd({
    ...
    description:
      ...
      `Book ${name} in ${location} — Experience authentic ${type} hospitality.`,
    parsed,
  });
  ...
  return (
    <div className="flex flex-col font-manrope bg-white w-full overflow-x-hidden">
      ...
      {/* 1. Hero Section */}
      <section className="relative min-h-[400px] sm:min-h-[450px] md:min-h-[500px] h-[60vh] max-h-[750px] md:max-h-[600px] w-full flex items-center justify-center overflow-hidden py-10 md:py-12">
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto text-center px-4 md:px-6 w-full flex flex-col items-center justify-center pb-10 md:pb-16">
          {/* Badges */}
          <div className="flex flex-wrap justify-center items-center gap-2 md:gap-3 mb-6 w-full max-w-[95%] md:max-w-none">
            {starCategory && (
              <span className="bg-gradient-to-r from-orange-500 to-amber-500 text-white text-[10px] sm:text-[11px] md:text-xs font-black px-3 py-1.5 md:px-3.5 md:py-1.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-1">
                <Star className="w-3 h-3 md:w-3.5 md:h-3.5 fill-current text-white" />
                {starCategory} Star {type}
              </span>
            )}
            <span className="bg-white/20 backdrop-blur-md text-white text-[10px] sm:text-[11px] md:text-xs font-bold px-3 py-1.5 md:px-3.5 md:py-1.5 rounded-full border border-white/30 flex items-center gap-1.5 shadow-sm">
              <ShieldCheck className="w-3 h-3 md:w-3.5 md:h-3.5 text-[#FF9530]" />{" "}
              Eco-Certified Property
            </span>
            <StaticDataBadge
              text="static data - need this data on the api"
              className="bg-white/20 text-white border-white/30 text-[10px] sm:text-[11px]"
            />
          </div>

          {/* Heading */}
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-black text-white mb-4 tracking-tight leading-tight drop-shadow-md px-2">
            {name}
          </h1>

          {/* Subtitle / Location */}
          <div className="text-xs sm:text-sm md:text-base lg:text-lg text-white/95 max-w-[800px] mx-auto mb-8 md:mb-10 leading-relaxed font-semibold drop-shadow-sm flex flex-wrap items-center justify-center gap-x-2 gap-y-2 px-2">
            <div className="flex items-start sm:items-center gap-1 text-left sm:text-center md:text-left">
              <MapPin className="inline-block w-3.5 h-3.5 md:w-5 md:h-5 text-[#FF9530] shrink-0 mt-[2px] sm:mt-0" />
              <span>{location}</span>
            </div>
          ...
          {/* CTA Buttons */}
          <div className="flex flex-row items-center justify-center gap-3 sm:gap-4 w-full sm:w-auto px-2 sm:px-0">
            <Link
              ...
              className="flex-1 sm:flex-none justify-center px-4 py-3.5 sm:px-8 sm:py-3.5 rounded-xl font-black text-[11px] sm:text-sm uppercase tracking-wider bg-gradient-to-r from-[#FF9530] to-[#FF8000] hover:from-[#FF8000] hover:to-[#F97316] text-white shadow-xl shadow-orange-500/30 hover:scale-[1.03] active:scale-[0.98] transition-all flex items-center gap-1.5 sm:gap-2"
            >
              <span>Book Now</span>
      ...
      <section className="py-12 md:py-16 px-6 md:px-12 max-w-[1200px] mx-auto w-full">
        <div className="text-center mb-12">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-3">
            <span className="text-[#FF9530] text-[11px] sm:text-xs font-black uppercase tracking-widest block whitespace-nowrap">
              Why Stay With Us
            </span>
            <StaticDataBadge
              text="static data - need this data on the api"
          ...
          <p className="text-gray-600 text-base max-w-[650px] mx-auto font-medium">
            Experience the unique blend of comfort and culture that makes {name}{" "}
            the top choice in {location}.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          <div className="bg-gray-50/80 p-6 md:p-8 rounded-[32px] border border-gray-100 hover:shadow-xl transition-all group flex flex-col items-center text-center">
            <div className="w-12 h-12 md:w-14 md:h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform">
              <Coffee className="w-6 h-6 md:w-7 md:h-7 text-[#FF9530]" />
            </div>
            <h3 className="text-lg md:text-2xl font-bold mb-2 md:mb-3 text-gray-900">
              Authentic Dining
            </h3>
            <p className="text-gray-600 leading-relaxed text-[13px] md:text-sm font-medium">
              Enjoy traditional meals prepared with fresh, locally sourced
              ingredients.
            </p>
          </div>
          ...
          <div className="bg-gray-50/80 p-6 md:p-8 rounded-[32px] border border-gray-100 hover:shadow-xl transition-all group flex flex-col items-center text-center">
            <div className="w-12 h-12 md:w-14 md:h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform">
              <MapPin className="w-6 h-6 md:w-7 md:h-7 text-[#FF9530]" />
            </div>
            <h3 className="text-lg md:text-2xl font-bold mb-2 md:mb-3 text-gray-900">
              Prime Location
            </h3>
            <p className="text-gray-600 leading-relaxed text-[13px] md:text-sm font-medium">
              Centrally located with easy access to top attractions and hubs.
            </p>
          </div>

          <div className="bg-gray-50/80 p-6 md:p-8 rounded-[32px] border border-gray-100 hover:shadow-xl transition-all group flex flex-col items-center text-center">
            <div className="w-12 h-12 md:w-14 md:h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform">
              <Star className="w-6 h-6 md:w-7 md:h-7 text-[#FF9530]" />
            </div>
            <h3 className="text-lg md:text-2xl font-bold mb-2 md:mb-3 text-gray-900">
              Guest First Policy
            </h3>
            <p className="text-gray-600 leading-relaxed text-[13px] md:text-sm font-medium">
              24/7 support and assistance ensuring a seamless stay.
            </p>
          </div>
        </div>
      </section>
