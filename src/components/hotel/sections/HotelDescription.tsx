...
export default function HotelDescription({ hotelData, entityKey }: Props) {
  ...
  const displayImages =
    allImages.length > 0
      ? allImages
      : [IMAGES.eventHero.src, IMAGES.bgSection.src, IMAGES.herobg.src];
  ...
  const keyStats = [
    {
      shortLabel: "TYPE",
      fullLabel: "PROPERTY TYPE",
      value: propertyType,
      icon: <Building className="w-4 h-4 text-[#FF9530]" />,
    },
    {
      shortLabel: "CATEGORY",
      fullLabel: "STAR CATEGORY",
      value: `${starCategory} Star`,
      icon: <Star className="w-4 h-4 text-[#FF9530]" />,
    },
    {
      shortLabel: "IN / OUT",
      fullLabel: "CHECK-IN / OUT",
      value: `${checkIn} / ${checkOut}`,
      icon: <Clock className="w-4 h-4 text-[#FF9530]" />,
    },
    {
      shortLabel: "ROOMS",
      fullLabel: "ROOMS & SUITES",
      value: `${roomCount} Rooms`,
      icon: <Bed className="w-4 h-4 text-[#FF9530]" />,
    },
  ];
  ...
  return (
    <section id="about-property" className="w-full bg-white font-manrope">
      ...
      <div className="bg-[#FAFBFD] rounded-[24px] sm:rounded-[28px] md:rounded-[36px] border border-gray-200/90 p-4 sm:p-6 md:p-8 lg:p-10 shadow-xs relative overflow-hidden space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 bg-white rounded-[20px] sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-gray-200/80 shadow-2xs flex flex-col justify-between space-y-4 md:space-y-6">
            ...
            {/* Action Buttons Row */}
            <div className="pt-2 md:pt-3 border-t border-gray-100 flex flex-col sm:flex-row items-center gap-3">
              {isLongDescription && (
                <button
                  type="button"
                  onClick={() => setIsExpanded((prev) => !prev)}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-black text-[#FF9530] bg-orange-50/90 border border-orange-200 hover:bg-orange-100 transition-all active:scale-95"
                >
                  <span>
                    {isExpanded ? "See Less" : "See More About Hotel"}
                  </span>
                  {isExpanded ? (
              ...
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold text-gray-700 bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-all active:scale-95"
              >
            ...
            {/* Feature Pills */}
            <div className="pt-3 flex flex-wrap gap-2 md:gap-2.5 text-[11px] sm:text-xs font-bold text-gray-700">
              <div className="flex items-center gap-1.5 bg-emerald-50/60 text-emerald-900 px-3 py-1.5 rounded-full border border-emerald-200/80">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span className="whitespace-nowrap">24/7 Front Desk</span>
              </div>
              <div className="flex items-center gap-1.5 bg-emerald-50/60 text-emerald-900 px-3 py-1.5 rounded-full border border-emerald-200/80">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span className="whitespace-nowrap">Free High-Speed Wi-Fi</span>
              </div>
              <div className="flex items-center gap-1.5 bg-emerald-50/60 text-emerald-900 px-3 py-1.5 rounded-full border border-emerald-200/80">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span className="whitespace-nowrap">Prime Location Access</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-28 w-full h-[280px] sm:h-[340px] lg:h-[380px]">
              <div
                onClick={() => handleOpenGallery(0)}
                className="group relative w-full h-full rounded-2xl overflow-hidden shadow-sm border border-gray-200 cursor-pointer"
              >
                <Image
                  src={featureImage}
                  alt={`${name} View`}
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent flex items-end justify-between p-3 sm:p-4 group-hover:bg-black/35 transition-colors gap-2">
                  <span className="text-white text-[10px] sm:text-xs font-black bg-black/50 backdrop-blur-md px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl border border-white/20 truncate max-w-[55%]">
                    {name} Overview
                  </span>

                  <button
                    type="button"
                    className="bg-gradient-to-r from-[#FF9530] to-[#FF8000] text-white text-[10px] sm:text-xs font-black px-2.5 py-1.5 sm:px-3.5 sm:py-1.5 rounded-lg sm:rounded-xl shadow-lg flex items-center justify-center gap-1.5 group-hover:scale-105 transition-all shrink-0"
                  >
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 shadow-2xs space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-100 pb-3 gap-2 sm:gap-0">
            <h3 className="text-[11px] sm:text-xs font-black text-gray-900 uppercase tracking-widest whitespace-nowrap">
              PROPERTY DETAILS
            </h3>
            <span className="text-[10px] sm:text-[11px] font-bold text-[#FF9530] bg-orange-50 px-2.5 py-1 sm:py-0.5 rounded-full border border-orange-200/80 whitespace-nowrap">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {keyStats.map((stat, idx) => (
              <div
                key={idx}
                className="bg-gray-50/90 p-3 sm:p-3.5 rounded-xl border border-gray-150/80 flex flex-col justify-between h-full"
              >
                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <div className="shrink-0">{stat.icon}</div>
                  <span className="text-[8px] sm:text-[9px] uppercase font-extrabold text-gray-400 tracking-wide leading-tight line-clamp-1 lg:line-clamp-none">
                    <span className="md:hidden">{stat.shortLabel}</span>
                    <span className="hidden md:inline">{stat.fullLabel}</span>
                  </span>
                </div>
                <p className="text-[11px] sm:text-xs md:text-sm font-black text-gray-900 truncate mt-auto">
                  {stat.value}
                </p>
              </div>
            ))}
