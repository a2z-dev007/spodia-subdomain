...
const HotelHeroPremium: React.FC<HotelHeroPremiumProps> = ({
  ...
  return (
    <section className="relative min-h-[380px] h-[52vh] max-h-[500px] w-full flex items-center justify-center overflow-hidden pb-12 font-manrope">
      ...
      <div className="max-w-[1200px] mx-auto text-center relative z-20 px-4 md:px-6">
        {/* Badges / Pill */}
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-4">
          {pillText && (
            <span className="bg-white/15 backdrop-blur-md text-white text-[10px] md:text-xs font-bold px-3.5 py-1.5 rounded-full border border-white/20 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          {badges && badges.length > 0
            ? badges.map((badge, i) => (
                <span
                  key={i}
                  className="bg-white/15 backdrop-blur-md text-white text-[10px] md:text-xs font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full border border-white/20 flex items-center gap-1.5"
                >
            ...
            : (
                <span className="bg-white/15 backdrop-blur-md text-white text-[10px] md:text-xs font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full border border-white/20 flex items-center gap-1.5">
                  <ShieldCheck className="w-3.5 h-3.5 text-[#FF9530]" /> Best Price Guarantee
                </span>
              )}
        </div>
